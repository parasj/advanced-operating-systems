...
extern void uthread_schedule(uthread_struct_t * (*kthread_best_sched_uthread)(kthread_runqueue_t *))
{
    ...
    #if 0
    fprintf(stderr, "uthread_schedule invoked !!n");
    #endif
        ...
        else
        {
            ...
            if (u_obj->scheduler_mode == 1){
                ...
                if (u_obj->now - u_obj->last_time >= CREDITS_BURN_TSLICE){
                    ...
                    printf("uthread %d CREDITS: %dn", u_obj->uthread_tid, u_obj->credits);
                }
            ...
            if(u_obj->uthread_tid == 1){
                printf("CYCLE_COUNT_1%dn",u_obj->cycle_count );
            }
    ...
    if(!(u_obj = kthread_best_sched_uthread(kthread_runq)))
    {
        ...
        {
            fprintf(stderr, "Quitting kthread (%d)n", k_ctx->cpuid);
    ...
    {
        fprintf(stderr, "uthread_init failed on kthread(%d)n", k_ctx->cpuid);
        exit(0);
    }
...
static int func(void *arg)
{
    ...
    printf("Thread %d createdn", u_info->num1);
    count = 0;
    while(count <= 0xffffff)
    {
        if(!(count % 5000000))
        printf("uthread(%d) => count : %dn", u_info->num1, count);
        count++;
    }
    #undef u_info
    return 0;
